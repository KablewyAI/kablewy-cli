...
The CLI does **not** send prompts, document contents, file paths, shell commands, stdout/stderr, environment variables, local audit logs, API keys, bearer tokens, cookies, or MCP auth headers as telemetry. In `kablewy agent`, local tool results are still sent as model context when needed to answer your request, for example when you ask the agent to read a file, list a directory, or inspect command output. To omit the Kablewy-specific client metadata entirely, set `KABLEWY_DISABLE_TELEMETRY=1`.
